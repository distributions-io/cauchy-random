...
*	Creates a matrix or array filled with Cauchy random numbers.
*
...
function random( dims, options ) {
	...
	x0 = typeof opts.x0 !== 'undefined' ? opts.x0 : 0;
	gamma = typeof opts.gamma !== 'undefined' ? opts.gamma : 1;

	if ( arguments.length === 0 ) {
		return number( x0, gamma, rand );
	}
	if ( isArray ) {
		ndims = dims.length;
		if ( ndims < 2 ) {
	...
	if ( ndims === 1 ) {
		...
		if ( dt === 'generic' ) {
			return array( len, x0, gamma, rand );
		}
		return typedarray( len, dt, x0, gamma, rand );
	}
	// Multidimensional data structures...
	if ( dt !== 'generic' ) {
		if ( ndims === 2 ) {
			return matrix( dims, dt, x0, gamma, rand );
		}
		// TODO: dstructs-ndarray support goes here. Until then, fall through to plain arrays...
	}
	return arrayarray( dims, x0, gamma, rand );
} // end FUNCTION random()
